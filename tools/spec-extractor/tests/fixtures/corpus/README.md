...
| `multipag-tributo-correto.txt` | Pagamento de tributo com código de barras: serviço `22`, forma de lançamento `11`, versão de lote `012` e um **Segmento O** no lugar do par A/B. Nenhum achado. O código de barras cai no ramo `resto10 != 0`, onde o dígito aceito é o do **módulo 10** — é o arquivo que prova que a redução por parcela está certa, porque um cálculo errado aqui vira falso positivo. |
| `multipag-tributo-dv-invalido.txt` | O mesmo arquivo com o dígito verificador do código de barras trocado por um valor que nem o módulo 10 nem o módulo 11 aceitam. Produz **um** achado, na coluna 021. |
